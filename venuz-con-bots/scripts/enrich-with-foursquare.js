...
async function main() {
    console.log('🚀 Iniciando Enriquecimiento de Datos (Foursquare Mode - JS)...');

    if (!FOURSQUARE_API_KEY) {
        console.warn('⚠️  ADVERTENCIA: No se encontró FOURSQUARE_API_KEY en .env.local');
        console.warn('⚠️  El script intentará correr pero no encontrará nada.');
        console.warn('⚠️  Por favor agrega la llave en .env.local y vuelve a correrlo.');
    }
    ...
    const { data: posts, error } = await supabase
        .from('content')
        .select('*')
        .is('rating', null) // Priorizamos lo que falta (Rating para Ojo de Dios)
    ...
    for (const post of posts) {
        ...
        if (lugar) {
            ...
            console.log('   ⚠️ No encontrado en Foursquare (o falta API Key).');
